...
def get_directory():
  ...
  if not os.path.exists(prefix + "directory_listing"):
    ...
    cats = cPickle.load(open(prefix + "taxonomy.pickle"))
  ...
  cat_tags = root.xpath("//div[@class='round-rect-green ' \
      or @class='round-rect-green no-content']/div[@class='inner']")
  ...
  if text:
    ...
    OUT = open(prefix + "taxonomy_categories.dat", "w")
    for key in categories:
      print >> OUT, key[0]
    OUT.close()
    OUT = open(prefix + "taxonomy_subcategories.dat", "w")
    for key in categories:
      for value in categories[key]:
        print >> OUT, key[0], value[0]
    OUT.close()
  return categories
...
def get_directory_listings(taxonomy):
  ...
  for cat in taxonomy:
    ...
    while True:
      ...
      #Dump to disk.
      OUT = open(prefix + "directory_listing/tech_%s_%s.html" % (url.split('/')[-2], str(pg)), "w")
      print >> OUT, page
      OUT.close()
      pg += 1
