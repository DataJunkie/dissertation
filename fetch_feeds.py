...
import urllib2, re, os, sys, time, socket, redis, cPickle, glob, logging
...
extract_titles should be in another file, but shoved it here for 
space.
"""
#TO DO:
...
def harvest_urls():
  ...
  for infile in glob.glob( os.path.join(directoryfiles, '*.html') ):
    ...
    category_text = root.xpath("//h2[@class='alternate']")[0].text
    res = patt.search(category_text)
    subcat = None
    try:
      cat, subcat = res.groups()
    except ValueError:
      cat = res.group(1)  #this cat has not subcats
...
def probe(urlbase, fileno):
  ...
  if len(links) > 0:
    ...
    if not url.startswith('http://') and not url.startswith('feed://') \
